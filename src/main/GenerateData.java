...
public class GenerateData {
    ...
    private final String orderLinePath;
    private final String newOrderPath;
    private final String customerPath;
    private final String districtPath;
    ...
    private static String nullConstant = "NULL";
    private static long lastTimeMS = 0;

    public GenerateData(String csvFilePath, int scaleFactor) {
        numWarehouses = scaleFactor;
        counter = new AtomicInteger();
        clock = new Clock(numWarehouses);

        // Create the necessary directories
        String csvDirectoryPath = csvFilePath + "/" + numWarehouses + "warehouses";
        File directory = new File(csvDirectoryPath);
        if (!directory.exists())
            directory.mkdirs();

        // Set the CSV file names
        this.warehousePath = csvDirectoryPath + "/" + CSVNAME_WAREHOUSE;
        this.orderLinePath = csvDirectoryPath + "/" + CSVNAME_ORDERLINE;
        this.newOrderPath = csvDirectoryPath + "/" + CSVNAME_NEWORDER;
        this.customerPath = csvDirectoryPath + "/" + CSVNAME_CUSTOMER;
        this.districtPath = csvDirectoryPath + "/" + CSVNAME_DISTRICT;
        this.historyPath = csvDirectoryPath + "/" + CSVNAME_HISTORY;
        this.stockPath = csvDirectoryPath + "/" + CSVNAME_STOCK;
        this.orderPath = csvDirectoryPath + "/" + CSVNAME_ORDER;
        this.itemPath = csvDirectoryPath + "/" + CSVNAME_ITEM;
    }
        ...
        try {
            ...
            for (int i = 1; i <= itemKount; i++) {
                ...
                if ((k % configCommitCount) == 0) {
                    ...
                    LOG.println(etStr.substring(0, 30) + "  Writing record " + k + " of " + t);
                    lastTimeMS = tmpTime;
                }
            } // end for
    ...
    private int loadStock(int whseKount, int itemKount) {
        ...
        try {
            ...
            LOG.println("Start of writing " + t + " records to the STOCK file");
            ...
            LOG.println(etStr.substring(0, 30) + "  Writing final records " + k + " of " + t);
            LOG.println("End Stock Load @  " + now);
...
//            outHist = new PrintWriter(new FileOutputStream(historyPath));
            ...
            // Write the headers
            outOrder.println(oorder.toHeader());
            outOrderLine.println(order_line.toHeader());
            outCust.println(customer.toHeader());
            outDist.println(district.toHeader());
            outWhse.println(warehouse.toHeader());

            now = new Date();

            baseRowCount = whseKount * distWhseKount * custDistKount;
            OrderLineCount = baseRowCount * 10; // Average of randomly generated value in [5,15], rounded up
            ...
            for (int w = 1; w <= whseKount; w++) {
                ...
                for (int d = 1; d <= distWhseKount; d++) {
                    ...
                    for (int c = 1; c <= custDistKount; c++) {
                        ...
                        customer.c_d_id = d;
                        customer.c_w_id = w;
                        customer.c_discount = RandomGenerator.randDoubleBetween(0, 5000) / 10000; // [0.0000 ... 0.5000]
...
//                        outHist.println(str);
                        ...
                        oorder.o_all_local = RandomGenerator.randDoubleBetween(0, 100) <= 5 ? 0 : 1;
...
//                            outNewOrder.println(str);
                        ...
                        for (int l = 1; l <= oorder.o_ol_cnt; l++) {
                            ...
                            if ((k % configCommitCount) == 0) {
                                ...
                                LOG.println(etStr.substring(0, 30) + "  Writing record " + k + " of " + total);
                                lastTimeMS = tmpTime;
                            }
                        } // end for [l]
                    } // end for [c]
            ...
            String etStr = "  Elasped Time(ms): "
                    ...
                    + "                    ";
            LOG.println(etStr.substring(0, 30) + "  Writing record " + k + " of " + total);
            LOG.println("End timestamped Data Load @  " + new Date());
            lastTimeMS = tmpTime;
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
...
//            outNewOrder.close();
    ...
    public void load() {
        ...
        LOG.println("------------- Data generation start = " + startDate + "-------------");
        long totalRows = loadTimestamped(numWarehouses, configDistPerWhse, configCustPerDist);
        ...
        LOG.println();
        LOG.println("------------- Data Generation Statistics --------------------");
        LOG.println("     Start Time = " + startDate);
        LOG.println("       End Time = " + endDate);
        LOG.println("       Run Time = " + (int) runTimeMS / 1000 + " Seconds");
        LOG.println("    Rows Loaded = " + totalRows + " Rows (not entirely accurate)");
        LOG.println("Rows Per Second = " + (totalRows / (runTimeMS / 1000)) + " Rows/Sec (not entirely accurate)");
        LOG.println(" Clock Start TS = " + new Timestamp(startTS));
        LOG.println("   Clock End TS = " + new Timestamp(finalTS));
        LOG.println("Total # of New Timestamps (Load Stage): " + counter.get());
        LOG.println("------------------------------------------------------");
    }

    /**
     * The input parameters should contain the following:
     *    - args[0]: Number of warehouses (scale factor)
     *    - args[1]: Path to write the CSV files
     */
    public static void main(String[] args) {
        int numWarehouses = Integer.parseInt(args[0]);
        String csvFilePath = args[1];

        GenerateData generator = new GenerateData(csvFilePath, numWarehouses);
        generator.load();
    }
